...
static void trigger_zigbee_local_reset(void)
{
    ESP_LOGW(TAG, "Button: triggering Zigbee local reset/rejoin");
    led_set_pattern(LED_PATTERN_PAIRING);
    if (esp_zb_lock_acquire(pdMS_TO_TICKS(1000))) {
        esp_zb_bdb_reset_via_local_action();
        esp_zb_lock_release();
    } else {
        ESP_LOGW(TAG, "Button: failed to acquire Zigbee lock for local reset");
    }
}

static void trigger_zigbee_factory_reset(void)
{
    ESP_LOGW(TAG, "Button: triggering Zigbee FACTORY reset");
    led_set_pattern(LED_PATTERN_PAIRING);
    if (esp_zb_lock_acquire(pdMS_TO_TICKS(1000))) {
        esp_zb_factory_reset();
        esp_zb_lock_release();
    } else {
        ESP_LOGW(TAG, "Button: failed to acquire Zigbee lock for factory reset");
    }
}
    ...
    while (1) {
        ...
        if (pressed) {
            ...
            if (was_pressed) {
                ...
                if (press_time >= BUTTON_FACTORY_RESET_MS) {
                    trigger_zigbee_factory_reset();
                } else if (press_time >= BUTTON_LONG_PRESS_MS) {
                    trigger_zigbee_local_reset();
